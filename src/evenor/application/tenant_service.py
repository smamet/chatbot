...
from evenor.domain.contracts.tenant_repository import TenantRepository
...
class TenantService:
    ...
    def delete_tenant(self, tenant_id: int, *, settings: Settings) -> bool:
        ...
        if not self._repo.delete(tenant_id):
            return False
        roots = (
            settings.data_root / "docs" / slug,
            settings.lancedb_root / slug,
            settings.data_root / "catalog" / slug,
            settings.data_root / "attachments" / slug,
            settings.data_root / "quotes" / slug,
            settings.data_root / "cac40" / slug,
            settings.data_root / "backups" / slug,
        )
        for root in roots:
            if root.exists():
