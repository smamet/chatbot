...
from evenor.adapters.persistence.orm import TestChatSessionRow
from evenor.application.customer_access_gate import parse_session_identity
    ...
    def upsert(
        ...
        *,
        ...
        if row is None:
            ...
            if created_by_user_id is not None and row.created_by_user_id is None:
    ...
    def list_recent(self, *, limit: int = 20) -> list[TestChatSessionRow]:
        rows = list(
            self._session.scalars(
                ...
                .options(joinedload(TestChatSessionRow.created_by_user))
        ...
        tracked = [
            row
            for row in rows
            if row.session_id.startswith(("test:", "email:", "whatsapp:"))
        ]
