...
from evenor.adapters.persistence.secrets import decrypt_text, encrypt_text
...
class SqlAlchemyTenantRepository:
    ...
    def delete(self, tenant_id: int) -> bool:
        ...
        if row is None:
            return False
        # FK-safe order: child audit/outbound → pending → mail → orders → core → tenant
        ...
        self._session.execute(delete(MailImapUidRow).where(MailImapUidRow.tenant_id == tenant_id))
        self._session.execute(delete(EmailThreadRow).where(EmailThreadRow.tenant_id == tenant_id))
        order_ids = list(
        ...
        self._session.execute(delete(ApiUsageDailyRow).where(ApiUsageDailyRow.tenant_id == tenant_id))
        self._session.execute(
            delete(DiskUsageDailyRow).where(DiskUsageDailyRow.tenant_id == tenant_id)
        )
        self._session.execute(delete(IntegrationRow).where(IntegrationRow.tenant_id == tenant_id))
        self._session.execute(
            delete(MailConnectionRow).where(MailConnectionRow.tenant_id == tenant_id)
        )
