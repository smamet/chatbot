...
from evenor.adapters.persistence.tenant_paths import tenant_catalog_dir, tenant_docs_dir, tenant_lancedb_dir
...
app = typer.Typer(no_args_is_help=True, help="Evenor CLI.")
...
def _resolve_password(password: str | None, *, command_hint: str) -> str:
    ...
    typer.echo(
        f"No TTY: pass --password/-p, e.g.\n  evenor {command_hint} -p 'your-password'",
        err=True,
    )
    raise typer.Exit(1)
    ...
    with factory() as session:
        ...
        if svc.find_by_email(email):
            typer.echo(
                ...
                f"  ./sail evenor user-set-password {email} -p 'new-password'",
                err=True,
            )
            raise typer.Exit(1)
...
def bot_flush_cmd(
    ...
    if backup_path is not None:
        typer.echo(f"Restore with: evenor bot-restore {slug} {backup_path} --yes")
    typer.echo("Done.")
...
def bot_restore_cmd(
    ...
    if not yes:
        if not sys.stdin.isatty():
            typer.echo(
                "No TTY: pass --yes / -y to confirm, e.g.\n"
                f"  evenor bot-restore {slug} {backup} --yes",
                err=True,
            )
            raise typer.Exit(1)
...
def mail_inbox_preview_cmd(
    ...
    from evenor.adapters.persistence.tenant_repository import SqlAlchemyTenantRepository
    from evenor.application.mail_inbox_preview_service import preview_tenant_inbox
...
def serve_cmd(
    ...
    uvicorn.run("evenor.interfaces.api.main:app", host=host, port=port, reload=reload)


@app.command("version")
def version_cmd() -> None:
    typer.echo("evenor 0.2.0")
