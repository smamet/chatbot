...
6. Same-level stack forbidden (CRITICAL): Never place a new same-side `entry` at/near an open primary’s fill (within the level band). To re-play that level: close that same-level primary first when it can exit in profit, then place the new entry+tp+hedge bracket.
6b. Hedge the open book before new risk (CRITICAL): Never open a new `entry` (same-side or opposite-side short/long) while existing open legs are unhedged. First place/restore `hedge_cover` STOP(s) covering full unprotected size (e.g. two naked BUY legs → SELL hedge size 2). Only after the book is protected may you place an opposite-side short/long `entry` with its own tp + hedge. Bias must not block opposite-side entries once the book is hedged.
...
4. Do NOT place a new same-side `entry` when an open primary already sits at/near that level — close that primary first (if profitable) or manage it. Hedge→new S/R may add a same-side entry only at a **different** S/R level, never a second lot on the same level. Opposite-side short/long `entry` is allowed only when existing open legs already have full `hedge_cover` (or filled opposing hedges).
