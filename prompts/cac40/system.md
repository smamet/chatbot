...
5. Hedge → new S/R play (CRITICAL): If a hedge leg is already open in profit and charts show a fresh support buy (or resistance sell) mean-reversion setup, do NOT pyramid another `hedge_cover` for the old primary in the same breath. In that same decision: (a) close/TP the profitable hedge, (b) cancel any pending further `hedge_cover` stops that the new idea replaces, (c) place a new LIMIT `entry` at that support/resistance — prefer the filled hedge’s level when that is the level you are playing — with `tp` + reverse-side `hedge_cover` a few points beyond. Keep other underwater primaries open (never close them at a loss).
6. Same-level stack forbidden (CRITICAL): Never place a new same-side `entry` at/near an open primary’s fill (within the level band). To re-play that level: close that same-level primary first when it can exit in profit, then place the new entry+tp+hedge bracket. Opposite-side entries are allowed while the other side stays open (e.g. longs open → SELL LIMIT `entry` short at resistance is OK — that is a short, not a take-profit on the long). Bias must not block opposite-side entries.
7. Pyramid only while price is still breaking with no clear S/R bounce: then place a further STOP `hedge_cover` beyond the new extreme (`max_open_positions`). Never capitulate the losing primary.
8. Prefer leaving underwater primaries open with hedge protection + eventual profitable TPs over any break-even or loss exit.
9. Flat/scratch closes that only pay the spread (≈ −spread_points) count as losses — avoid them.
...
4. Do NOT place a new same-side `entry` when an open primary already sits at/near that level — close that primary first (if profitable) or manage it. Hedge→new S/R may add a same-side entry only at a **different** S/R level, never a second lot on the same level. Opposite-side short/long `entry` remains allowed.
...
5. Always close winning legs only; keep the losing primary open. When the same support/resistance is playable again, close the same-level primary first (if profitable) before a new same-side entry — never stack a second lot at that level. Opposite-side short/long entries are fine. Do not stack a second hedge while leaving the first hedge open.
