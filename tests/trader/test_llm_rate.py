...
def test_llm_rate_from_form_15m() -> None:
    n, unit, bars = TraderConfig.llm_rate_from_form(every_n=3, unit="15m")
    assert (n, unit, bars) == (3, "15m", 3)


def test_llm_rate_from_form_hours() -> None:
    n, unit, bars = TraderConfig.llm_rate_from_form(every_n=2, unit="1h")
    assert (n, unit, bars) == (2, "1h", 8)


def test_resolve_llm_every_bars() -> None:
    cfg = TraderConfig(llm_every_n=1, llm_every_unit="1h")
    assert cfg.resolve_llm_every_bars() == 4
    cfg = TraderConfig(llm_every_n=4, llm_every_unit="15m")
    assert cfg.resolve_llm_every_bars() == 4
