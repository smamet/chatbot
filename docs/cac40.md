...
| `/dashboard/bots/{slug}/cac40/live/report` | Live results (decision browser + PnL) |
...
| `worker-cac40-live` | Background live cycles |

## Live trading (dashboard)

On **CAC40 Backtest** → **Live trading**:

1. Add one or more IG connectors (Connectors tab). Use a label (e.g. Demo CFD). Prefer an IG **DEMO** connector to practice.
...
4. **Off** = worker skips the bot. **Live** = real working orders on selected IG accounts (each connector’s DEMO or LIVE env). There is no simulated paper ledger — use DEMO for paper trading.
5. **View results** opens the live report (same decision browser as backtest: charts sent to Gemini + LLM answer). Recent cycles also appear under **Live cycles** on the CAC40 page. **Review & sync book** previews local vs IG before applying.
6. **Arm Live** does not call Gemini immediately — it waits for the next 15m Paris slot. Auto slots still respect Adaptive/Fixed LLM schedule. **Run cycle now** runs immediately and **Force LLM** (default on) bypasses that schedule (still skips on stale OHLC / unresolved book desync).
7. Disarming (Off) stops new cycles but **does not cancel** IG working orders already placed.
...
- **Sync:** **Live never fills from candles** — IG is source of truth. Live polls `GET /workingorders` every cycle; `GET /positions` is event-driven (arm, WO change, after mirror, flatten, or once per new 15m bar). Opens/closes bind by IG `dealId`. Desync triggers replace-rebuild of the open book (and quarantines phantom local closes). Dashboard **Review & sync book** previews the same replace, then applies on confirm. Unresolved desync skips the LLM. Auto-sync changes are logged as anomalies.
...
Config lives under `data/cac40/{slug}/live/` (`live_config.json`, `status.json`, `state.json`, `decisions_log.json`, `sync_log.json`, `journal/{cycle}/cycle.json` + charts).
...
- Background worker (`worker-cac40-ohlc`, poll `CAC40_OHLC_POLL_SECONDS` default 900) tops up tenants with active CAC40 + IG + existing CSV (no cron bootstrap of empty files). Skips bots armed for Live (their live worker tops up instead).
