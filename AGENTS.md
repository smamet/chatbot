...
- **Trader bots (Evenor)**: `tenants.bot_type=trader` + `config_json.trader` (symbol/epic/FM/profile). Package `chatbot.trader/`; UI `?tab=trading`; data `data/trader/{slug}/`. Workers `worker-trader-ohlc` / `worker-trader-live` / `worker-trader-stream` (Lightstreamer OHLC ticks + TRADE wake-up book sync; REST for dealing/backfill). Docs: [docs/trader.md](docs/trader.md). Run `./sail migrate` after pull for `026_tenant_bot_type`.
