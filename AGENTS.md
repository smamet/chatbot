...
- **Trader bots (Evenor)**: `tenants.bot_type=trader` + `config_json.trader` (symbol/epic/FM/profile). Package `chatbot.trader/`; UI `?tab=trading`; data `data/trader/{slug}/` (migrates from `data/cac40/`). Workers `worker-trader-ohlc` / `worker-trader-live` / `worker-trader-stream` (Lightstreamer OHLC ticks + TRADE wake-up book sync; REST for dealing/backfill). Docs: [docs/trader.md](docs/trader.md). Run `./sail migrate` after pull for `026_tenant_bot_type`.
...
Compose services: `db` (MySQL), `api`, `worker-automation`, `worker-mail`, `worker-catalog`, `worker-trader-ohlc`, `worker-trader-live`, `worker-trader-stream`; optional `caddy` (`./sail up -d --profile caddy`). App data bind-mounts `./data` → `/app/data` by default (docs, catalog, LanceDB, attachments). Root `.env` is loaded via `env_file`; `DATABASE_URL` is overridden to MySQL inside containers.

**Dev:** `docker-compose.override.yml` bind-mounts `./src` — edit Python without `./sail build`. API runs with `--reload`. Rebuild only after dependency/Dockerfile changes. Restart workers after code changes: `./sail restart worker-automation` / `./sail restart worker-mail` / `./sail restart worker-catalog` / `./sail restart worker-trader-ohlc` / `./sail restart worker-trader-live` / `./sail restart worker-trader-stream`.
...
**Workers:** `HOOK_POLL_SECONDS`, `MAIL_POLL_SECONDS`, `CATALOG_POLL_SECONDS` (catalog worker poll; default 300), `TRADER_OHLC_POLL_SECONDS` (IG OHLC top-up; default 900), `TRADER_LIVE_POLL_SECONDS` (live cycle poll; default 60), `TRADER_STREAM_LOOP_SECONDS` (Lightstreamer supervisor; default 5), `STREAM_TICK_STALE_SECONDS` (default 120). Legacy `CAC40_*` aliases accepted. Catalog worker also runs daily disk snapshots when due.
